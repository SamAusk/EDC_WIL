<footer>
    <div class="slab primary puff trim-top">
        <div class="inner">
            <h2 class="visuallyhidden">Footer</h2>
            <div class="flex col-5 p-hide">
                <div>
                    <h3 tabindex="91">Study areas</h3>
                    <ul>
                        <li><a href="https://www.griffith.edu.au/study/architecture-construction-planning">Architecture,
                                construction and planning</a></li>
                        <li><a href="https://www.griffith.edu.au/study/business-government">Business and government</a>
                        </li>
                        <li><a href="https://www.griffith.edu.au/study/criminology-law">Criminology and law</a></li>
                        <li><a href="https://www.griffith.edu.au/study/education">Education</a></li>
                        <li><a href="https://www.griffith.edu.au/study/engineering-it-aviation">Engineering, <abbr
                                    title="Information Technology">IT</abbr> and aviation</a></li>
                        <li><a href="https://www.griffith.edu.au/study/humanities-languages">Humanities, languages and
                                social science</a></li>
                        <li><a href="https://www.griffith.edu.au/study/health">Medicine, dentistry and health</a></li>
                        <li><a href="https://www.griffith.edu.au/study/music">Music and performing arts</a></li>
                        <li><a href="https://www.griffith.edu.au/study/science-environment">Science and environment</a>
                        </li>
                        <li><a href="https://www.griffith.edu.au/study/visual-creative-arts">Visual and creative
                                arts</a></li>
                    </ul>
                </div>
                <div>
                    <h3 tabindex="92">Campuses</h3>
                    <ul>
                        <li><a
                                href="https://www.griffith.edu.au/about-griffith/campuses-facilities/digital?city=online">Digital
                                campus</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities/gold-coast">Gold
                                Coast campus</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities/logan">Logan
                                campus</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities/mt-gravatt">Mount
                                Gravatt campus</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities/nathan">Nathan
                                campus</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities/south-bank">South
                                Bank campus</a></li>
                    </ul>
                    <h3 tabindex="93">Getting to Griffith</h3>
                    <ul>
                        <li><a href="https://www.griffith.edu.au/transport/parking#cycling">Cycling</a></li>
                        <li><a href="https://www.griffith.edu.au/transport/intercampus-bus"
                                title="Intercampus bus (Nathan-Mt Gravatt)">Intercampus bus</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities">Maps</a></li>
                        <li><a href="https://www.griffith.edu.au/transport/parking">Parking</a></li>
                    </ul>
                </div>
                <div>
                    <h3 tabindex="94">On campus</h3>
                    <ul>
                        <li><a href="https://www.griffith.edu.au/accommodation">Accommodation</a></li>
                        <li><a href="https://www.griffith.edu.au/sport/sport-and-recreation/aquatic-and-fitness-centre">Aquatic
                                and Fitness Centre</a></li>
                        <li><a href="https://www.griffith.edu.au/clubs">Clubs</a></li>
                        <li><a href="https://www.griffithcollege.edu.au">Griffith College</a></li>
                        <li><a href="https://www.griffith.edu.au/health/clinics">Health Clinics</a></li>
                        <li><a href="https://www.griffith.edu.au/library">Library</a></li>
                        <li><a
                                href="https://www.griffith.edu.au/student-services/health-and-medical-services">Medical</a>
                        </li>
                        <li><a href="https://www.griffith.edu.au/shops">Shops</a></li>
                        <li><a href="https://www.griffith.edu.au/sport">Sport</a></li>
                        <li><a href="https://www.griffith.edu.au/whats-on">What's on</a></li>
                    </ul>
                </div>
                <div>
                    <h3 tabindex="95">Support</h3>
                    <ul>
                        <li><a href="https://www.griffith.edu.au/childcare">Childcare</a></li>
                        <li><a href="https://www.griffith.edu.au/students/enrolment-timetables-fees/paying-your-fees">Paying
                                your fees</a></li>
                        <li><a href="https://app.secure.griffith.edu.au/griffithpay/">Griffith Pay</a></li>
                        <li><a href="https://www.griffith.edu.au/griffith-honours-college">Griffith Honours College</a>
                        </li>
                        <li>
                            <a href="https://www.griffith.edu.au/gumurrii">
                                <abbr title="Griffith University Murri and Torres Strait Islander">GUMURRII</abbr>
                                Student Success Unit</a>
                        </li>
                        <li><a href="https://www.griffith.edu.au/digital-solutions/it-service-centre"><abbr
                                    title="Information Technology">IT</abbr> Service Centre</a></li>
                        <li><a href="https://www.griffith.edu.au/international">International</a></li>
                        <li><a href="https://www.griffith.edu.au/printing">Printing services</a></li>
                        <li><a href="https://www.griffith.edu.au/room-bookings">Room bookings</a></li>
                        <li><a href="https://www.griffith.edu.au/safe-campuses">Safe campuses</a></li>
                        <li><a href="https://www.griffith.edu.au/student-support">Student support</a></li>
                        <li><a href="https://www.griffith.edu.au/students/assessment-exams-grades">Assessment, exams and
                                grades</a></li>
                    </ul>
                </div>
                <div>
                    <h3 tabindex="96">Academic groups</h3>
                    <ul>
                        <li><a href="https://www.griffith.edu.au/arts-education-law">Arts, Education and Law</a></li>
                        <li><a href="https://www.griffith.edu.au/griffith-business-school">Griffith Business School</a>
                        </li>
                        <li><a href="https://www.griffith.edu.au/griffith-health">Griffith Health</a></li>
                        <li><a href="https://www.griffith.edu.au/griffith-sciences">Griffith Sciences</a></li>
                    </ul>
                    <h3 tabindex="97">Contact us</h3>
                    <ul>
                        <li><a href="https://www.griffith.edu.au/emergency-information">Emergency information</a></li>
                        <li><a
                                href="https://www.griffith.edu.au/marketing-communications/media-and-communications">Media</a>
                        </li>
                        <li><a href="https://www.griffith.edu.au/security">Security</a></li>
                        <li><a href="https://www.griffith.edu.au/about-griffith/campuses-facilities">Addresses</a></li>
                        <li><a href="https://www.griffith.edu.au/search">Phone</a></li>
                        <li><a
                                href="https://www.griffith.edu.au/about-griffith/corporate-governance/complaints-and-grievances">Complaints
                                and grievances</a></li>
                    </ul>
                </div>
            </div>
            <!-- /.col-5-->
            <div class="flex col-5 t-col-3 m-visible">
                <div class="flex a-left a-middle m-a-center m-order-1">
                    <h3><a href="https://www.griffith.edu.au/ask-us">Need help?</a></h3>
                    <ul>
                        <li class="ask-us"><a href="https://www.griffith.edu.au/ask-us" class="ir">Ask us</a>
                    </ul>
                </div>
                <div class="flex a-right m-a-center m-order-3">
                    <h3><a href="https://www.griffith.edu.au/feedback/_nocache">Provide feedback</a></h3>
                </div>
            </div>
            <!-- /.col-5-->
            <div class="flex col-2 m-visible">
                <div>
                    <h3 class="visuallyhidden">Disclaimer</h3>
                    <ul class="flex a-left m-a-center m-small">
                        <li><a
                                href="https://www.griffith.edu.au/about-griffith/corporate-governance/plans-publications/griffith-university-privacy-plan">Privacy
                                plan</a>
                        <li class="pipe"><a href="https://www.griffith.edu.au/copyright-matters">Copyright matters</a>
                        <li class="pipe">
                            <a
                                href="http://cricos.education.gov.au/Institution/InstitutionDetails.aspx?ProviderCode=00233E">
                                <abbr
                                    title="Commonwealth Register of Institutions and Courses for Overseas Students">CRICOS</abbr>
                                Provider - 00233E</a>
                    </ul>
                </div>
                <div>
                    <h3 class="visuallyhidden">Locations</h3>
                    <ul class="flex a-right m-a-center m-small">
                        <li class="gold-coast"><a
                                href="https://www.griffith.edu.au/about-griffith/campuses-facilities/gold-coast">Gold
                                Coast</a>
                        <li class="logan pipe"><a
                                href="https://www.griffith.edu.au/about-griffith/campuses-facilities/logan">Logan</a>
                        <li class="brisbane pipe"><a
                                href="https://www.griffith.edu.au/about-griffith/campuses-facilities">Brisbane</a>
                        <li class="australia ndash"><a
                                href="https://www.griffith.edu.au/life-at-griffith/australia">Australia</a>
                    </ul>
                </div>
            </div>
            <!-- /.col-2-->
            <div class="flex a-center m-visible">
                <!--
        <div class="flex a-middle gu10 t-gu12 m-a-center">

        <ul class="flex a-around gu2 m-gu5">
        <li class="aboriginal"><a href="https://www.griffith.edu.au/about-griffith/first-peoples" class="ir">Aboriginal</a>
        <li class="torres-strait-islander"><a href="https://www.griffith.edu.au/about-griffith/first-peoples" class="ir">Torres Strait Islander</a>
        </ul>
        <p class="gu10 m-a-center"><a href="https://www.griffith.edu.au/about-griffith/first-peoples">Griffith University campuses are situated on the land of the Yugarabul, Yuggera, Jagera, Turrbal, Yugambeh and Kombumerri peoples.</a></p>

        </div>
    -->
                <h3><a href="https://www.griffith.edu.au/about-griffith/first-peoples">First Peoples of
                        Australia</a>&nbsp;</h3>
                <ul class="flex a-left">
                    <li class="aboriginal"><a href="https://www.griffith.edu.au/about-griffith/first-peoples"
                            class="ir">Aboriginal</a>&nbsp;
                    <li class="torres-strait-islander"><a
                            href="https://www.griffith.edu.au/about-griffith/first-peoples" class="ir">Torres Strait
                            Islander</a>
                </ul>
            </div>
        </div>
    </div>
    <!-- /.slab /.inner -->
</footer>